...
/**
 * Function to initialize docker_info plugin.
 *
 * @param ins     Pointer to flb_input_instance
 * @param config  Pointer to flb_config
 *
 * @return struct flb_in_dinfo_config* Pointer to the plugin's
                                        ...
                                        struct flb_config *config)
{
    int ret;
    const char *tmp;
    struct flb_in_dinfo_config *ctx;

    ctx = flb_calloc(1, sizeof(struct flb_in_dinfo_config));
    if (!ctx) {
        flb_errno();
        return NULL;
    }
...
/**
 * Function to destroy docker_info plugin.
 *
 * @param ctx  Pointer to flb_in_dinfo_config
 *
 * @return int 0
 */
int dinfo_config_destroy(struct flb_in_dinfo_config *ctx)
{
    if (ctx->buf) {
        flb_free(ctx->buf);
    }
