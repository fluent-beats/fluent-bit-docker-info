 ...
 * @return int 0 on success, -1 on failure
 */
static int dinfo_unix_socket_connect(struct flb_in_dinfo_config *ctx)
{
    unsigned long len;
 ...
 * @return int Always returns success
 */
static int cb_dinfo_collect(struct flb_input_instance *ins,
                             struct flb_config *config, void *in_context)
{
    struct flb_in_dinfo_config *ctx = in_context;
 ...
 * @return int 0 on success, -1 on failure
 */
static int in_dinfo_init(struct flb_input_instance *ins,
                          struct flb_config *config, void *data)
{
    struct flb_in_dinfo_config *ctx = NULL;
    (void) data;

    /* Allocate space for the configuration */
    ctx = dinfo_config_init(ins, config);
    if (!ctx) {
        return -1;
    }
    ctx->ins = ins;
    ...
    ctx->coll_id = flb_input_set_collector_time(ins, cb_dinfo_collect,
                                                ctx->collect_interval,
                                                0, config);
    if(ctx->coll_id < 0){
        flb_plg_error(ctx->ins, "could not set collector for IN_DOCKER_INFO plugin");
        dinfo_config_destroy(ctx);
        return -1;
    }
 ...
 *
 * @param data    Pointer cast to flb_in_dinfo_config
 * @param config  Unused
 *
 * @return int    Always returns 0
 */
static int in_dinfo_exit(void *data, struct flb_config *config)
{
    (void) config;
    struct flb_in_dinfo_config *ctx = data;
...
static struct flb_config_map config_map[] = {
    ...
    },
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_in_dinfo_config, unix_path),
     "Define Docker unix socket path to read events"
    },
    {
     FLB_CONFIG_MAP_STR, "containers_path", DEFAULT_DOCKER_LIB_ROOT,
     0, FLB_TRUE, offsetof(struct flb_in_dinfo_config, containers_path),
     "Define Docker unix socket path to read events"
    },
    {
    ...
    },
    {
     ...
     0, FLB_TRUE, offsetof(struct flb_in_dinfo_config, key),
